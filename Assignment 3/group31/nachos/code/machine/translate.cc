...
// from the virtual address
  ...
  if (vpn >= NachOSpageTableSize) {
      ...
      return AddressErrorException;
  } else if (NachOSpageTable[vpn].valid == FALSE) {
      if(currentThread->space->validpages < currentThread->space->GetNumPages()) {
            entry = &NachOSpageTable[vpn];
            
            unsigned int size = (currentThread->space)->GetNumPages() * PageSize;
            
            unsigned int sizeToRead = PageSize;
            
            executable = fileSystem->Open((currentThread->space)->filename);
            
            noffH = currentThread->space->noffH;
            
            

            phypgnum = (int *)freepagelist->Remove();
            if(phypgnum != NULL) {
                entry->physicalPage = *phypgnum;
            } else {
                entry->physicalPage = (nextpagetoallocate++);
            }
            
            pageFrame = entry->physicalPage;
            //bzero(&machine->mainMemory[pageFrame*PageSize], PageSize);
            for(int j = 0; j < PageSize; ++j) {
              machine->mainMemory[pageFrame*PageSize + j] = 0;
            }
            if(vpn==(currentThread->space->GetNumPages() - 1) ) {
               sizeToRead = size - vpn * PageSize;
            
            }
            

            executable->ReadAt(&(machine->mainMemory[pageFrame * PageSize]),sizeToRead, noffH.code.inFileAddr + vpn*PageSize);
